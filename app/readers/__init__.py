...
def _discover_builtin_readers() -> list[str]:
    ...
    for py_file in readers_dir.glob("*.py"):
        ...
        try:
            module = importlib.import_module(f".{module_name}", package="app.readers")
...
def get_reader(name_or_path: str) -> Callable[[str], tuple]:
    ...
    # Built-in reader
    try:
        module = importlib.import_module(f".{name_or_path}", package="app.readers")
        if not hasattr(module, "reader"):
            raise ValueError(f"Reader module '{name_or_path}' has no 'reader' function")
        return module.reader
    except ImportError as e:
        raise ValueError(f"Could not load built-in reader '{name_or_path}': {e}")
