...
def load_reader_function(script_path: str) -> Callable[[str], list]:
    ...
    if not hasattr(module, 'reader'):
        raise ValueError(
            f"Reader script must define a 'reader(filename)' function")
...
def gcps_to_affine(gcps: list, img_width: int,
                   img_height: int) -> tuple[Affine, CRS]:
    ...
    if len(gcps) < 3:
        raise ValueError(
            "At least 3 GCPs are required to compute an affine transform")

    # Convert lat/lon to Web Mercator
    lats = [gcp[2] for gcp in gcps]
    lons = [gcp[3] for gcp in gcps]

    xs_mercator, ys_mercator = transform_coords(
        WGS84, WEB_MERCATOR, lons, lats)
...
def load_image_with_reader(
    ...
    if not isinstance(image_data, np.ndarray):
        raise ValueError(
            f"Reader must return numpy array for image_data, got {
                type(image_data)}")

    if image_data.ndim not in (2, 3):
        raise ValueError(
            f"Image data must be 2D (grayscale) or 3D (RGB/RGBA), got {image_data.ndim}D")

    # Validate GCPs
    if not gcps or len(gcps) < 3:
        raise ValueError(
            f"Reader returned insufficient GCPs ({
                len(gcps) if gcps else 0}). Need at least 3.")

    for i, gcp in enumerate(gcps):
        if len(gcp) != 4:
            raise ValueError(
                f"GCP {i} has {
                    len(gcp)} elements, expected 4: [x, y, lat, lon]")
    ...
    if image_data.ndim == 2:
        ...
        gray = image_data.astype(
            np.uint8) if image_data.dtype != np.uint8 else image_data
        rgba_array[:, :, 0] = gray
        rgba_array[:, :, 1] = gray
        rgba_array[:, :, 2] = gray
        rgba_array[:, :, 3] = 255
        ...
        # RGB -> RGBA
        rgba_array = np.zeros((img_height, img_width, 4), dtype=np.uint8)
        rgb = image_data.astype(
            np.uint8) if image_data.dtype != np.uint8 else image_data
        rgba_array[:, :, :3] = rgb
        rgba_array[:, :, 3] = 255
    elif image_data.shape[2] == 4:
        # Already RGBA
        rgba_array = image_data.astype(
            np.uint8) if image_data.dtype != np.uint8 else image_data
    else:
        raise ValueError(
            f"Image must have 1, 3, or 4 channels, got {
                image_data.shape[2]}")
...
def compute_bounds_from_affine(
        affine: Affine, width: int, height: int) -> tuple[float, float, float, float]:
